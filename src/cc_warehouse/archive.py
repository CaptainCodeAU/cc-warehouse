...
def write_subagent(
    ...
    *,
    ...
    render_options: render.RenderOptions | None = None,
) -> SubagentResult:
    ...
    NO markdown or HTML is generated by default: sub-agents are archived, not
    rendered, unless `render_options` is given (ticket 28.11, the flag this
    docstring used to record as future work). When given, the same four files a
    session gets -- transcript.md, transcript.compact.md, conversation.html,
    conversation.compact.html -- are written beside this sub-agent's own JSONL,
    via `store.write_if_changed` (idempotent, so a re-run with unchanged content
    writes nothing) rather than the session writer's whole incremental-skip and
    manifest machinery, which a sub-agent has no manifest to drive anyway.
    ...
    # Ticket 28.11. THE PAYLOAD THAT RENDERS IS THE ONE THAT SURVIVED (ticket 29's
    # lesson, applied here too): on a refusal this reads the JSONL back off disk
    # rather than rendering the declined `data`, so the markdown/HTML never
    # describe a payload that lost the replace-if-larger check above.
    # `write_if_changed` makes every one of the four writes idempotent, so this
    # runs unconditionally rather than needing its own skip-if-current check the
    # way `write_session_folder` needs one for its five files.
    if render_options is not None:
        rendered = jsonl.read_bytes() if (refused_smaller or refused_equal_size) else data
        full_md, compact_md = render.render_markdown(rendered, render_options)
        full_html, compact_html = render.render_html(rendered, render_options)
        md_name, compact_md_name, html_name, compact_html_name, _manifest_name = GENERATED_NAMES
        wrote |= store.write_if_changed(directory / md_name, full_md)
        wrote |= store.write_if_changed(directory / compact_md_name, compact_md)
        wrote |= store.write_if_changed(directory / html_name, full_html)
        wrote |= store.write_if_changed(directory / compact_html_name, compact_html)
