...
- Tag parity (every DONE-annotated slice must carry its milestone tag; added 2026-07-18 after this drift recurred; SUFFIX-AWARE since 2026-07-24, when `^[0-9]+` collapsed the split tickets 12a and 12b to "12" and reported slice-12 missing twice, a tag that correctly does not exist because ticket 12 is SUPERSEDED): !`R=$(git rev-parse --show-toplevel); M=""; for t in "$R"/harness/tickets/*.md; do grep -qE 'DONE 20[0-9][0-9]-[0-9][0-9]-[0-9][0-9]' "$t" || continue; n=$(basename "$t" | grep -oE '^[0-9]+[a-z]?'); [ -n "$n" ] || continue; git -C "$R" rev-parse -q --verify "refs/tags/slice-$n" >/dev/null 2>&1 || M="$M slice-$n"; done; [ -n "$M" ] && echo "MISSING milestone tag(s):$M (DONE slice, no tag)" || echo "(every DONE-annotated slice carries its milestone tag)"`
...
- README verb list vs the CLI (added 2026-07-24 when a Verbs block was added to README; it must be the SAME user-facing set the -h help prints, which is the _VERBS table in cli.py, NOT the dispatcher, since internal verbs like notify are dispatched but deliberately absent from -h. Computed from source, no Python spawn; scoped to the _VERBS block ALONE so the content-flag tuples lower in cli.py are not mistaken for verbs; no literal backticks in the command, which broke the first version): !`R=$(git rev-parse --show-toplevel); CLI=$(sed -n '/^_VERBS/,/^)/p' "$R/src/cc_warehouse/cli.py" | grep -oE '\("[a-z]+"' | tr -d '("' | sort -u); RM=$(grep -oE '^ccw [a-z]+' "$R/README.md" | sed 's/ccw //' | sort -u); if [ -z "$RM" ]; then echo "(README has no Verbs block)"; else D=$(comm -3 <(echo "$CLI") <(echo "$RM") | tr -d '\t' | tr '\n' ' '); [ -n "$(echo $D)" ] && echo "README VERB LIST DRIFT (differs from cli.py _VERBS):$D" || echo "(README verb list matches the -h help exactly)"; fi`
- CHANGELOG milestone table vs git tags (added 2026-07-24; WIDENED 2026-08-02 after it reported slice-19d and slice-19f as missing while they were named in the file: the suffix class was [ab], written when 12a/12b were the only lettered tags, so it could not SEE a 19d token and would have cried wolf forever, training a future session to ignore a probe that was otherwise correct. It also missed ticket-NN tags entirely. Both classes are matched now. The seeded table must name every real slice tag and invent none, and must NOT claim a version heading while distribution is deferred, DESIGN 15. NO literal backticks in the command: the tags are matched as bare slice-NN tokens, since the CHANGELOG code-spans them and a literal backtick here closes the probe block and breaks the whole command, which is exactly the bug this replaces): !`R=$(git rev-parse --show-toplevel); [ -f "$R/CHANGELOG.md" ] || { echo "(no CHANGELOG.md)"; exit 0; }; GT=$(git -C "$R" tag -l 'slice-*' -l 'ticket-*' | sort); CT=$(grep -oE '(slice|ticket)-[0-9]+[a-z]?' "$R/CHANGELOG.md" | sort -u); MISS=$(comm -23 <(echo "$GT") <(echo "$CT") | tr '\n' ' '); INV=$(comm -13 <(echo "$GT") <(echo "$CT") | grep -vx 'slice-12' | tr '\n' ' '); VER=$(grep -cE '^## \[?v?[0-9]+\.[0-9]+' "$R/CHANGELOG.md"); OUT=""; [ -n "$(echo $MISS)" ] && OUT="$OUT missing-tags:$MISS"; [ -n "$(echo $INV)" ] && OUT="$OUT invented-tags:$INV"; [ "$VER" -gt 0 ] && OUT="$OUT claims-a-release-while-deferred"; [ -n "$OUT" ] && echo "CHANGELOG DRIFT:$OUT" || echo "(CHANGELOG names every slice tag, invents none, claims no release)"`
